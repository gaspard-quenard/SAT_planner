...
def generate_figures():
    ...
    for file_name in csv_files_names:
        ...
        data.loc[(data['sat_plan_size'] == 0), 'sat_total_run_time'] = 0
        ...
        ax1.set_title("Time spent by planners on problems of benchmark \"{benchmark_name}\" to find a plan".format(
            benchmark_name=benchmark_name))
        ax1.set_xlabel("Problem name")
        ax1.set_ylabel("Time (s)")

        # Shift the bar of hsp a little to the left and the bar of sat a little to the right
        ...
        ax1.bar(abcisse, ordinate_sat, facecolor='r',
                label="SAT", transform=shiftBarRight, width=0.2)
        ...
        ax2.set_title("Size of plan generated by planners on problems of benchmark \"{benchmark_name}\"".format(
            benchmark_name=benchmark_name))
        ...
        ax2.bar(abcisse, ordinate_hsp, facecolor='b',
                label="HSP", transform=shiftBarLeft, width=0.2)
        ax2.bar(abcisse, ordinate_sat, facecolor='r',
                label="SAT", transform=shiftBarRight, width=0.2)

        plt.legend(loc='upper left')

        path_figure = os.path.join(PATH_OUTPUT, "figure_result_{benchmark_name}.png".format(
            benchmark_name=benchmark_name))

        logging.info("Save figure {figure_name}".format(
            figure_name=path_figure))
